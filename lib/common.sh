...
################################################################################
# provision_alert_channel()
# Description:
#     Write a Grafana contact point and notification policy so the existing
#     alert rules have somewhere to fire.
#
# Args:
#     $1 -> channel (currently: telegram)
#     $2 -> target; for telegram, "<bot-token>:<chat-id>"
#
# Behaviors:
#     - Writes provisioning/alerting/contactpoints.yaml under the monitoring
#       assets, which Grafana reads at startup.
#     - Routes everything to the one contact point. Per-severity routing is a
#       thing to add when somebody asks for it, not before.
#     - Does NOT touch the alert rules. Their thresholds are already better than
#       the published guides; only delivery was missing.
#
# Returns:
#     0 on success; 1 on bad input or a write failure.
################################################################################
provision_alert_channel() {
    local channel="$1" target="$2"
    local dir="$N8N_DIR/monitoring/grafana/provisioning/alerting"

    case "$channel" in
        telegram) ;;
        *) log ERROR "Unsupported alert channel: $channel"; return 1 ;;
    esac

    # "<bot-token>:<chat-id>" — the bot token itself contains a colon, so split
    # on the LAST one.
    local bot_token="${target%:*}"
    local chat_id="${target##*:}"
    if [[ -z "$bot_token" || -z "$chat_id" ]]; then
        log ERROR "--alert-target must look like '<bot-token>:<chat-id>'."
        return 1
    fi

    mkdir -p "$dir" || { log ERROR "Cannot create $dir"; return 1; }

    cat > "$dir/contactpoints.yaml" <<EOF
# Generated by n8n_manager.sh --alerts telegram
# The alert rules alongside this file already carry sensible thresholds; this is
# the delivery they were missing.
apiVersion: 1
...
EOF
    ...
    chmod 600 "$dir/contactpoints.yaml" 2>/dev/null || true
    log INFO "Grafana alerts will be delivered to Telegram chat ${chat_id}"
    log INFO "Contact point: $dir/contactpoints.yaml (contains the bot token; mode 600)"
    return 0
}
